...
{
	//struct drm_connector *connector = connector;//remove on kernel 5.10
	struct ili9881c *ctx = panel_to_ili9881c(panel);
	...
	{
		mode = drm_mode_duplicate(connector->dev, &asus_ili9881c_default_mode_7inch);
		if (!mode)
		{
			dev_err(&ctx->dsi->dev, "failed to add mode %ux%ux@%u\n",
				asus_ili9881c_default_mode_7inch.hdisplay,
				asus_ili9881c_default_mode_7inch.vdisplay,
				drm_mode_vrefresh(&asus_ili9881c_default_mode_7inch));
			return -ENOMEM;
		}
		connector->display_info.width_mm = 88;
		connector->display_info.height_mm = 153;
	}
	else if(lcd_size_flag[ctx->dsi_id]==2)
	{
		mode = drm_mode_duplicate(connector->dev, &ili9881c_default_mode_10inch);
		if (!mode)
		{
			dev_err(&ctx->dsi->dev, "failed to add mode %ux%ux@%u\n",
				ili9881c_default_mode_10inch.hdisplay,
				ili9881c_default_mode_10inch.vdisplay,
				drm_mode_vrefresh(&ili9881c_default_mode_10inch));
			return -ENOMEM;
		}
