...
bool lt9211_is_tinker3n(void)
{
	if (!g_lt9211) {
		printk(KERN_INFO "%s g_lt9211 is null!\n", __func__);
		return false;
	}
	return g_lt9211->is_tinker3n;
}
EXPORT_SYMBOL_GPL(lt9211_is_tinker3n);
			   ...
			   struct lt9211_data *data)
{
	...
	printk(KERN_INFO "%s +\n", __func__);
	data->is_tinker3n = of_property_read_bool(np, "Tinker3N");
	data->register_33 = 0;
	data->register_35 = 0;
	data->register_3f = 0;
	data->register_40 = 0;

	if(!of_property_read_bool(np, "enable-overlay") && data->is_tinker3n) {
