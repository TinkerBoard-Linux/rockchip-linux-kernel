...
static int ov5647_sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on)	{
		dev_info(&client->dev, "OV5647 power on\n");
		...
		dev_info(&client->dev, "OV5647 power off\n");
...
static int ov5647_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	dev_info(&client->dev, "%s: enable %d", __func__, enable);
	if (enable)
			  ...
			  struct v4l2_subdev_format *fmt)
{
	...
	dev_info(&client->dev, "ov5647_set_fmt() w:%d, h:%d\n", mode->width, mode->height);
	mutex_unlock(&ov5647->lock);
...
static int ov5647_probe(struct i2c_client *client)
{
	...
	if (!IS_ERR_OR_NULL(sensor->pins_default)) {
		...
		dev_err(dev, "pins_default is error or null");
	}
	...
	if (IS_ERR(sensor->reset_gpio))
		dev_warn(dev, "Failed to get reset_gpio\n");
	msleep(500);
